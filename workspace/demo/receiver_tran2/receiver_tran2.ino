...
void loop(){
    ...
    Serial.print("text:");
    Serial.println(txt);
    while(!parse_str.startsWith("OK",0)){
      Serial.println("Pass");
      char txt[64] = "";
      radio.read(&txt, sizeof(txt));
      parse_str = String(txt);
    }
    parse_str = parse_str.substring(2, parse_str.length());    
    for (int i=0; i<5; i++){
      ...
      if(commapos != -1){
        ...
        if(i == 2){
          ...
          pressureStr = String(int(pressure/100));          
    ...
    while(!parse_str2.startsWith("SS",0)){
      // Serial.println("Pass");
      char txt2[64] = "";
      radio.read(&txt2, sizeof(txt2));
      parse_str2 = String(txt2);
    }
    parse_str2 = parse_str2.substring(2, parse_str2.length());    
    // Serial.print("text:");
    // Serial.println(txt2);
    for (int j=5; j<9; j++){
